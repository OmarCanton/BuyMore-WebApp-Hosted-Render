...
export default function ProductCheck () {
    ...
    const addComment = async (e) => {
        ...
        if(isLoggedIn) {
            try {
                const response = await axios.post('http://localhost:3000/addProductComment', commentBody, {withCredentials: true})
    ...
    const addRating = async () => {
        if( isLoggedIn) {
            try{
                const response = await axios.post('http://localhost:3000/addRating', ratingBody, {withCredentials: true})
                toast.success(response.data.msg, {
                    style: {
                        backgroundColor: 'black',
                        color: 'white',
